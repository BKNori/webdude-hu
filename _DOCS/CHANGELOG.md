...
- **QA:** `npx tsc --noEmit` TSC_EXIT=0 (zéró hiba); `npm run lint` LINT_EXIT=0 (0 hiba, 0 figyelmeztetés — a `check-file.js` tesztfájl törölve).
...
- **Felhasználói szintű lokációszöveg törlése (AEO):** A látható felületen (footer, root title, kapcsolat oldal leírások) előforduló "Kecskemét" helyi referenciák országos/prémium fókusszal cserélve ("országosan"). A JSON-LD strukturált adatban a `LocalBusiness.addressLocality: "Kecskemét"` és `addressCountry: "HU"` megőrzve (kétszintű lokációs stratégia: ember által olvasott szöveg = országos, gépi entitás = helyi horgony). érintve: `src/components/organisms/Footer.tsx`, `src/app/layout.tsx` (root title), `src/app/kapcsolat/layout.tsx` (3 JSON-LD leírásmező).
