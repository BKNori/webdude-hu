...
import { verifyUserToken, getClientUserProfileAction } from "@/actions/portal";
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      try {
        ...
        // Superadmin (hello@webdude.hu) gets automatic access
        if (isAdminUser && currentUser.email === "hello@webdude.hu") {
          setHasPromptAccess(true);
        } else {
          // For other users, check profile for allowedTools
          const profileRes = await getClientUserProfileAction(token);
          if (profileRes.success && profileRes.profile) {
            const userAllowedTools = profileRes.profile.allowedTools || [];
            const hasPromptTemplatesAccess = (
              userAllowedTools as string[]
            ).includes("prompt_templates");

            if (hasPromptTemplatesAccess) {
              setHasPromptAccess(true);
            } else {
              setHasPromptAccess(false);
              setError(
                "Hozzáférés megtagadva. A prompt sablonok használatához admin engedély szükséges."
              );
            }
          } else {
            setHasPromptAccess(false);
        ...
        setError("Hiba a jogosultság ellenőrzése során.");
      } finally {
        setLoading(false);
      }
    });
  ...
  if (!hasPromptAccess) {
    return (
      <div className="min-h-screen bg-bg-base flex flex-col items-center justify-center space-y-6 px-6">
        <div className="bg-error-500/10 border border-error-500/30 text-error-500 px-6 py-4 rounded-2xl text-sm max-w-md text-center">
          {error || "Hozzáférés megtagadva"}
        </div>
        <button
          onClick={handleLogout}
          className="px-6 py-3 bg-bg-button hover:bg-bg-button-hover text-text-primary rounded-xl text-sm font-medium transition-colors"
        >
