...
- **Metadata export** → TILOS Client Componentben (`page.tsx`-en ha `"use client"` van) → kiszervezni `layout.tsx`-be (Server Component)
- **JSON-LD Schema** → TILOS Client Componentben → kiszervezni `layout.tsx`-be (Server Component)
...
Kártya/panel:     #0f172a  →  bg-[#0f172a] (bg-surface)
